...
def parse_arguments():
    ...
    parser.add_argument("--epochs", "-e", default=35, type=int)
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        if val_loss < best_val_loss:
            ...
            with open("{}_best.pth".format(args.name), "wb") as fp:
    ...
    with open("{}_best.pth".format(args.name), "rb") as fp:
        best_state_dict = torch.load(fp, map_location="cpu")
        model.load_state_dict(best_state_dict)

    test_predictions = predict(model, test_dataloader, device)
    with open("{}_test_predictions.pkl".format(args.name), "wb") as fp:
        pickle.dump({"image_names": test_dataset.image_names,
                     "landmarks": test_predictions}, fp)

    create_submission(args.data, test_predictions, "{}_submit.csv".format(args.name))
